<div class="pageHeader">
	<form id="pagerForm" onsubmit="return navTabSearch(this);" action="/admin/profit/${states?if_exists}" method="post">
				...
				<#if beans?exists && beans?size gt 0>
					<#list beans as ls>
						<tr target="sid_user" rel="${(ls.id)?if_exists}">
							<td>
								<a target="navTab" href="/admin/account/profit?uid=${(ls.profile.id)?if_exists}">${(ls.profile.nickName)?if_exists}</a>
							</td>
							<td>
								<#if (ls.type)==0>
									推荐注册
