...
<div class="pageContent">
	<form method="post" action="/admin/subject/submit" enctype="multipart/form-data" class="pageForm required-validate" onsubmit="return iframeCallback(this, navTabAjaxDone);">
		...
		<input type="hidden" name="domain1"  value="${(bean.domain)?if_exists}">
		<input type="hidden" name="qrcode"  value="${(bean.qrcode)?if_exists}">
		...
		<div class="pageFormContent" layoutH="56">
			...
			<p>
				<label>域名：</label>
				<input name="domain" type="text" value="${(bean.domain)?if_exists}" size="30"/>
			</p>
