...
<div class="pageHeader">
	<form id="pagerForm" method="post" action="/admin/buy/querydog" onsubmit="return dwzSearch(this, 'dialog');">
		...
		<div class="searchBar">
			<ul class="searchContent">
				<li>
					<label>宠物名:</label>
					<input class="textInput" name="dogName" value="${dogName?if_exists}" type="text">
				</li>	  
			</ul>
			<div class="subBar">
				<ul>
			...
			<#if beans?exists && beans?size gt 0>
				<#list beans as ls>
					<tr>
						...
						<td>${(ls.chip)?if_exists}</td>
						<td>${(ls.category.name)?if_exists}</td>
						<td>
							<a class="btnSelect" href="javascript:$.bringBack({id:'${(ls.id)?if_exists}', chip:'${(ls.chip)?default("0")}'})" title="查找带回">选择</a>
						</td>
					</tr>
				</#list>
			</#if>
