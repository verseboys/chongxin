<div class="pageHeader">
	<form id="pagerForm" onsubmit="return navTabSearch(this);" action="/admin/product" method="post">
		...
		<div class="searchBar">
			<table class="searchContent">
				<tr>
					<td>
						产品名：<input type="text" name="title" value="${title?if_exists}"/>
					</td>
				</tr>
			</table>
			<div class="subBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="/admin/product/add" target="navTab"><span>添加</span></a></li>
			<li><a class="delete" href="/admin/product/delete?id={sid_user}" target="ajaxTodo" title="确定要删除吗?"><span>删除</span></a></li>
			<li><a class="edit" href="/admin/product/update?id={sid_user}" target="navTab"><span>修改</span></a></li>
		</ul>
	</div>
	<div id="w_list_print">
		<table class="table" width="100%" layoutH="138">
			<thead>
				<tr>
					<th width="100" align="center">产品</th>
					<th width="100" align="center">价格</th>
					<th width="100" align="center">单位</th>
					<th width="100" align="center">销量</th>
					<th width="100" align="center">上市时间</th>
				...
				<#if beans?exists && beans?size gt 0>
					<#list beans as ls>
						<tr target="sid_user" rel="${(ls.id)?if_exists}">
							<td>${(ls.product)?if_exists}</td>
							<td>${(ls.price)?if_exists}</td>
							<td>${(ls.unit)?if_exists}</td>
							<td>${(ls.count)?if_exists}</td>
							<td>${(ls.createdtimeStr)?if_exists}</td>
							<td>
								<a title="修改" target="navTab" href="/admin/product/update?id=${(ls.id)?if_exists}" class="btnEdit">修改</a>
								<a title="删除" target="ajaxTodo" href="/admin/product/delete?id=${(ls.id)?if_exists}" class="btnDel">删除</a>
							</td>
						</tr>
					</#list>
				</#if>
