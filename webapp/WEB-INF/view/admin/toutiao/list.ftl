<div class="pageHeader">
	<form id="pagerForm" onsubmit="return navTabSearch(this);" action="/admin/toutiao" method="post">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="delete" id="delete" href="/admin/toutiao/delete?id={sid_user}" target="ajaxTodo" title="确定要删除吗?"><span>删除头条</span></a></li>
		</ul>
	</div>
	<div id="w_list_print">
		<table class="table" width="100%" layoutH="138">
			<thead>
				<tr>
					...
					<th width="100" align="center">浏览次数</th>
				...
				<#if beans?exists && beans?size gt 0>
					<#list beans as ls>
						<tr target="sid_user" rel="${(ls.id)?if_exists}">
							<td>${(ls.sucai.title)?if_exists}</td>
							<td>${(ls.sucai.logo)?if_exists}</td>
							<td>${(ls.sucai.auth)?if_exists}</td>
							<td>${(ls.viewCount)?default(0)}</td>
							<td>${(ls.publishedStr)?if_exists}</td>
							<td>
								<a title="删除头条" target="ajaxTodo" href="/admin/toutiao/delete?id=${(ls.id)?if_exists}" class="btnDel">删除头条</a>
							</td>
						</tr>
					</#list>
				</#if>
